...
class TestVectorTransFormFunctions(TestCase):
    ...
    def setUp(self) -> None:
        self.vectors = [Vector3D([0, 0, 0]), Vector3D([1, 1, 1]), Vector3D([2, 3, 4])]
    ...
    def get_transformations(self):
        ...
        transformations.append(
            transformations[0].chain(transformations[1]).chain(transformations[2])
        )
    ...
    def test_rotation(self):
        ...
        self.assert_almost_equal_vec(
            rotation.apply(Vector3D([1, 0.0, -1.0])), Vector3D([0, 1.0, 1.0])
        )

    def test_scale(self):
        """Test scaling"""
        for scale_amount in (0.1, 1.0, 10, 100):
            scale = Transformation.scale(scale_amount)
            for vector in (self.vectors[0], self.vectors[1], self.vectors[2]):
                self.assert_almost_equal_vec(scale.apply(vector), vector * scale_amount)
    ...
    def test_inverse(self):
        ...
        for transformation in self.get_transformations():
            for vector in self.vectors:
                self.assert_almost_equal_vec(
                    transformation.apply_inverse(transformation.apply(vector)), vector
                )
